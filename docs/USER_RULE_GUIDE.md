...
## Cách nhanh nhất: tạo rule trực tiếp từ Note

1. Sau khi import, tìm dòng có nội dung mới trong bảng tài sản.
2. Di chuột lên dòng và chọn biểu tượng **Tạo rule từ dòng này** ở cột cuối.
3. Tool chuyển tới **Configure → Detection rules** và tự điền hostname, trường
   `Note`, nội dung cần khớp, classification `Cần xác minh` và remediation mẫu.
4. Rút gọn nội dung tự điền thành từ khóa có thể tái sử dụng. Không nên giữ nguyên
   cả một câu dài nếu các máy khác chỉ chứa tên tool hoặc artifact.
5. Chọn **Thử trên dữ liệu hiện tại**. Kiểm tra:
   - tổng số dòng khớp;
   - số dòng thực sự đổi kết luận;
   - số server/client bị ảnh hưởng;
   - kết luận trước và sau;
   - evidence đã khớp;
   - cảnh báo chồng lấn với rule hiện có.
6. Nút **Lưu & áp dụng** chỉ được bật sau khi bản rule hiện tại đã dry-run thành
   công. Sau khi lưu, toàn bộ dữ liệu đang mở được phân tích lại ngay.

## Tạo rule thủ công cho từ khóa mới
