...
class RuleEngineTests(unittest.TestCase):
    ...
    def test_proxy_tool_creates_traceable_review_finding(self) -> None:
        findings = evaluate_asset(
            {"type": "client", "hostname": "PC-01", "software": "Proxifier 4"}, self.pack["rules"]
        )
    ...
    def test_negated_malware_text_does_not_create_false_positive(self) -> None:
        findings = evaluate_asset(
            {"result": "No malware detected", "notes": "Clean"}, self.pack["rules"]
        )
    ...
    def test_rule_can_be_disabled_by_preset_configuration(self) -> None:
        ...
        self.assertFalse(
            any(item["ruleId"] == "PROXY_TOOL_REVIEW" for item in result["clients"][0]["findings"])
        )
        ...
        findings = [
            {
                ...
                "evidence": [{"field": "result", "value": "Chưa kết luận"}],
            },
    ...
    def test_custom_rule_from_metadata_is_applied_immediately(self) -> None:
        custom = validate_rule(
            {
                "id": "CUSTOM_INTERNAL_PROXY",
                "name": "Internal proxy review",
                "severity": "medium",
        ...
        finding = next(
            ...
            if item["ruleId"] == "CUSTOM_INTERNAL_PROXY"
        )
            ...
            [
                {
                    ...
                    "name": "A",
                    "classification": "needs_review",
                    "conditions": {"fields": ["notes"], "containsAny": ["Proxifier"]},
                },
                {
                    "id": "B",
                    "name": "B",
                    "classification": "anomaly",
                    "conditions": {"fields": ["notes", "software"], "containsAny": ["proxifier"]},
                },
                {
                    "id": "C",
                    "name": "C",
                    "classification": "anomaly",
                    "conditions": {"fields": ["result"], "containsAny": ["proxifier"]},
                },
            ]
        )
