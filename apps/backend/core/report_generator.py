...
class ReportBuilder:
    ...
    def build(self) -> Any:
        ...
        document, using_template, _template_mode, _resolved_template = _create_base_document(self.template_path)
        ...
        data = evaluate_payload(self._filter_data())
...
def _set_cell_text(
    ...
    *,
    ...
    _format_cell(cell, bold=bold, centered=centered)
...
def _resolve_heading_numbering_id(paragraph: Any) -> int | None:
    ...
    for abstract_num in numbering_root.findall(qn("w:abstractNum")):
        ...
        if abstract_id:
            abstract_nums[abstract_id] = abstract_num
    ...
    for num in numbering_root.findall(qn("w:num")):
        ...
        abstract_num = abstract_nums.get(abstract_id)
        if abstract_num is None:
            continue
        if not _looks_like_heading_numbering(abstract_num):
            continue
        try:
...
def _looks_like_heading_numbering(abstract_num: Any) -> bool:
    modules = _docx_modules()
    qn = modules["qn"]
    ...
    for level in abstract_num.findall(qn("w:lvl")):
        ...
        if level_index is None:
            continue
        try:
            level_number = int(level_index)
        except ValueError:
            continue

        num_fmt = level.find(qn("w:numFmt"))
        lvl_text = level.find(qn("w:lvlText"))
        if num_fmt is None or lvl_text is None:
            continue
    ...
    return (
        levels.get(0, ("", ""))[0] == "decimal"
        and "%1" in levels.get(0, ("", ""))[1]
        and levels.get(1, ("", ""))[0] == "decimal"
        and "%1.%2" in levels.get(1, ("", ""))[1]
        and levels.get(2, ("", ""))[0] == "decimal"
        and "%1.%2.%3" in levels.get(2, ("", ""))[1]
    )
