...
ALLOWED_CLASSIFICATIONS = {"informational", "needs_review", "anomaly"}
...
def validate_rule(rule: dict[str, Any]) -> dict[str, Any]:
    ...
    allowed_fields = {"type", "hostname", "ip", "os", "result", "notes", "software"}
...
def evaluate_payload(payload: dict[str, Any], *, disabled_rule_ids: list[str] | None = None) -> dict[str, Any]:
    ...
    for section in ("servers", "clients"):
        for asset in result.get(section, []):
...
def assessment_text(findings: list[dict[str, Any]] | None) -> str:
    ...
    if any(item.get("classification") == "anomaly" for item in evidence_backed):
        return STANDARD_ASSESSMENTS["anomaly"]
    if any(item.get("classification") == "needs_review" for item in evidence_backed):
        return STANDARD_ASSESSMENTS["needs_review"]
    return STANDARD_ASSESSMENTS["clean"]
