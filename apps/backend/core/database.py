        ...
        *,
        ...
        self._execute_commit(
            ...
            (
                ...
                max(0, int(duration_ms)), error_code, _now_iso(),
            ),
        )
        return rid

    def list_reports(self, limit: int | None = 50) -> list[dict]:
    ...
    def dashboard_summary(
        ...
        *,
        ...
        activity = self._fetch_all(
            """SELECT created_at FROM report_history
               WHERE status = 'success' AND created_at >= ? AND created_at <= ?
               ORDER BY created_at""",
            (start_iso, now_iso),
        )
        ...
        return {
            ...
            "series": series,
            "recent": self.list_reports(recent_limit),
        }
