...
const jsonResponse = (payload, ok = true) => ({ ok, status: ok ? 200 : 400, json: vi.fn().mockResolvedValue(payload) });
...
describe('ReporterProvider API workflow', () => {
  it('opens Configure with a rule draft sourced from an imported note', async () => {
    ...
    expect(result.current.ruleDraftSource).toEqual(expect.objectContaining({
      row: 0, field: 'notes', value: 'Acme Relay detected', hostname: 'pc-note',
    }));
  });
  ...
  it('imports a file with suggested column mapping', async () => {
    fetch
      .mockResolvedValueOnce(jsonResponse({
        columns: ['Host', 'IP'], suggestedMapping: { Host: 'hostname', IP: 'ip' },
        sheets: [], headerRow: 0,
      }))
      .mockResolvedValueOnce(jsonResponse({
        rows: [{ type: 'server', hostname: 'srv-01', ip: '10.0.0.1' }],
        counts: { servers: 1, clients: 0, total: 1 }, payload: {}, previewText: 'ok',
      }));
    const { result } = renderHook(() => useReporter(), { wrapper });
  ...
  it('blocks download when validation reports a severe error', async () => {
    fetch
      .mockResolvedValueOnce(jsonResponse({ rows: [{ type: 'server', hostname: '' }], payload: {} }))
      .mockResolvedValueOnce(jsonResponse({ valid: false, issues: [{ row: 1, level: 'error' }], summary: { errorRows: 1 } }));
    const { result } = renderHook(() => useReporter(), { wrapper });
    await act(() => result.current.loadSample());
  ...
  it('validates, generates and downloads a report', async () => {
    fetch
      .mockResolvedValueOnce(jsonResponse({ rows: [{ type: 'server', hostname: 'srv-01' }], payload: {} }))
      .mockResolvedValueOnce(jsonResponse({ valid: true, issues: [], summary: { totalRows: 1, validRows: 1 } }))
      .mockResolvedValueOnce(jsonResponse({
      ...
      .mockResolvedValueOnce(jsonResponse({
        job: {
          id: 'job-1', status: 'completed', phase: 'completed', progress: 100,
          filename: 'assessment.docx', reportId: 'report-1',
        },
      }))
  ...
  it('saves presets and updates templates through their API contracts', async () => {
    ...
    expect(fetch).toHaveBeenCalledWith('/api/templates/tpl-1', expect.objectContaining({ method: 'PATCH' }));
  });

  it('surfaces a failed background job without attempting a download', async () => {
    fetch
      .mockResolvedValueOnce(jsonResponse({ rows: [{ type: 'client', hostname: 'pc-01' }], payload: {} }))
      .mockResolvedValueOnce(jsonResponse({ valid: true, issues: [], summary: { totalRows: 1 } }))
      .mockResolvedValueOnce(jsonResponse({ job: { id: 'job-fail', status: 'queued' }, deduplicated: false }))
      .mockResolvedValueOnce(jsonResponse({
        job: { id: 'job-fail', status: 'failed', phase: 'failed', errorMessage: 'Generation failed safely' },
      }));
    const { result } = renderHook(() => useReporter(), { wrapper });
    await act(() => result.current.loadSample());
    let failure;
    await act(async () => {
      try { await result.current.generateReport(); } catch (error) { failure = error; }
    });
  ...
  it('sends disabled rule ids with the queued report request', async () => {
    fetch
      .mockResolvedValueOnce(jsonResponse({ rows: [{ type: 'client', hostname: 'pc-01' }], payload: {} }))
      .mockResolvedValueOnce(jsonResponse({ valid: true, issues: [], summary: { totalRows: 1 } }))
      .mockResolvedValueOnce(jsonResponse({ job: { id: 'job-rule', status: 'queued' }, deduplicated: false }))
      .mockResolvedValueOnce(jsonResponse({ job: { id: 'job-rule', status: 'cancelled', phase: 'cancelled' } }));
    const { result } = renderHook(() => useReporter(), { wrapper });
    await act(() => result.current.loadSample());
    act(() => result.current.setReportSettings({
    ...
    expect(JSON.parse(request[1].body).metadata.ruleSettings.disabledRuleIds).toEqual(['PROXY_TOOL_REVIEW']);
  });

  it('keeps existing rows after a 422 import and clears the error after a successful retry', async () => {
    fetch
      .mockResolvedValueOnce(jsonResponse({ rows: [{ type: 'server', hostname: 'srv-existing' }], payload: {} }))
      .mockResolvedValueOnce(statusResponse(422, { detail: 'Header row is invalid' }))
      .mockResolvedValueOnce(jsonResponse({
        columns: ['Host'], suggestedMapping: { Host: 'hostname' }, sheets: [], headerRow: 0,
      }))
      .mockResolvedValueOnce(jsonResponse({
        rows: [{ type: 'client', hostname: 'pc-recovered' }],
        counts: { servers: 0, clients: 1, total: 1 }, payload: {}, previewText: 'recovered',
      }));
    const { result } = renderHook(() => useReporter(), { wrapper });
    await act(() => result.current.loadSample());
  ...
  it('recovers preview state after a server error without losing imported rows', async () => {
    fetch
      .mockResolvedValueOnce(jsonResponse({ rows: [{ type: 'client', hostname: 'pc-preview' }], payload: {} }))
  ...
  it('marks an asynchronous Preview current and sends it for explicit promotion', async () => {
    fetch
      .mockResolvedValueOnce(jsonResponse({ rows: [{ type: 'server', hostname: 'srv-preview-job' }], payload: {} }))
      .mockResolvedValueOnce(jsonResponse({
        previewId: 'preview-1', jobId: 'preview-job-1', status: 'ready',
        signature: 'signature-1', progress: 100, phase: 'completed', cacheMode: 'deterministic',
      }))
      .mockResolvedValueOnce(statusResponse(200))
      .mockResolvedValueOnce(jsonResponse({ valid: true, issues: [], summary: { totalRows: 1 } }))
      .mockResolvedValueOnce(jsonResponse({ job: { id: 'report-from-preview', status: 'cancelled' }, deduplicated: false }))
      .mockResolvedValueOnce(jsonResponse({ job: { id: 'report-from-preview', status: 'cancelled', phase: 'cancelled' } }));
    const { result } = renderHook(() => useReporter(), { wrapper });
    await act(() => result.current.loadSample());
    await act(() => result.current.previewDocx());
  ...
  it('marks a completed Preview stale when report settings changed during generation', async () => {
    ...
    fetch
      .mockResolvedValueOnce(jsonResponse({ rows: [{ type: 'client', hostname: 'pc-race' }], payload: {} }))
      .mockResolvedValueOnce(jsonResponse({
        previewId: 'preview-race', jobId: 'job-race', status: 'ready',
        signature: 'signature-race', progress: 100, phase: 'completed', cacheMode: 'deterministic',
      }))
      .mockReturnValueOnce(contentPromise);
    const { result } = renderHook(() => useReporter(), { wrapper });
    await act(() => result.current.loadSample());
    let pending;
    act(() => { pending = result.current.previewDocx(); });
    await waitFor(() => expect(fetch.mock.calls.some(([url]) => String(url).endsWith('/content'))).toBe(true));
    act(() => result.current.setReportSettings({
      ...result.current.reportSettings, organization: 'Changed while Preview was running',
    }));
  ...
  it('cancels a Preview only through the explicit DELETE action', async () => {
    fetch
      .mockResolvedValueOnce(jsonResponse({ rows: [{ type: 'client', hostname: 'pc-cancel-preview' }], payload: {} }))
      .mockResolvedValueOnce(jsonResponse({
        previewId: 'preview-cancel', jobId: 'job-preview-cancel', status: 'ready',
        signature: 'signature-cancel', progress: 100, phase: 'completed', cacheMode: 'deterministic',
      }))
      .mockResolvedValueOnce(statusResponse(200))
  ...
  it('allows retry after a 429 queue response and does not duplicate the failed request', async () => {
    fetch
      .mockResolvedValueOnce(jsonResponse({ rows: [{ type: 'server', hostname: 'srv-busy' }], payload: {} }))
      .mockResolvedValueOnce(jsonResponse({ valid: true, issues: [], summary: { totalRows: 1 } }))
      .mockResolvedValueOnce(statusResponse(429, { detail: 'Job queue is full' }))
      .mockResolvedValueOnce(jsonResponse({ valid: true, issues: [], summary: { totalRows: 1 } }))
      .mockResolvedValueOnce(jsonResponse({ job: { id: 'job-retry', status: 'queued' }, deduplicated: false }))
      .mockResolvedValueOnce(jsonResponse({ job: { id: 'job-retry', status: 'cancelled', phase: 'cancelled' } }));
    const { result } = renderHook(() => useReporter(), { wrapper });
    await act(() => result.current.loadSample());
  ...
  it('retries transient job polling failures and completes the original job', async () => {
    ...
    try {
      fetch
        .mockResolvedValueOnce(jsonResponse({ rows: [{ type: 'server', hostname: 'srv-poll' }], payload: {} }))
        .mockResolvedValueOnce(jsonResponse({ valid: true, issues: [], summary: { totalRows: 1 } }))
        .mockResolvedValueOnce(jsonResponse({ job: { id: 'job-poll', status: 'queued' }, deduplicated: false }))
        .mockRejectedValueOnce(new TypeError('Network interrupted'))
        .mockResolvedValueOnce(statusResponse(503))
        .mockResolvedValueOnce(jsonResponse({
          job: { id: 'job-poll', status: 'completed', filename: 'poll.docx', reportId: 'report-poll' },
        }))
        .mockResolvedValueOnce(statusResponse(200));
      const { result } = renderHook(() => useReporter(), { wrapper });
      await act(() => result.current.loadSample());
  ...
  it('releases an unavailable job after retry exhaustion so the user can generate again', async () => {
    ...
    try {
      fetch
        .mockResolvedValueOnce(jsonResponse({ rows: [{ type: 'client', hostname: 'pc-retry' }], payload: {} }))
        .mockResolvedValueOnce(jsonResponse({ valid: true, issues: [], summary: { totalRows: 1 } }))
        .mockResolvedValueOnce(jsonResponse({ job: { id: 'job-lost', status: 'queued' }, deduplicated: false }))
        ...
        .mockResolvedValueOnce(jsonResponse({ valid: true, issues: [], summary: { totalRows: 1 } }))
        .mockResolvedValueOnce(jsonResponse({ job: { id: 'job-new', status: 'queued' }, deduplicated: false }))
        .mockResolvedValueOnce(jsonResponse({ job: { id: 'job-new', status: 'cancelled', phase: 'cancelled' } }));
      const { result } = renderHook(() => useReporter(), { wrapper });
      await act(() => result.current.loadSample());
      let failure;
      await act(async () => {
