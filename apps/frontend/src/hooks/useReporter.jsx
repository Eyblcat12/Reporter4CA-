...
function reducer(state, action) {
  switch (action.type) {
    ...
    case 'SET_ROWS': {
      ...
      return { ...state, rows, counts: recountRows(rows) };
    }
    case 'ADD_ROW': {
...
export function ReporterProvider({ children }) {
  ...
  const generateReport = useCallback(async () => {
    ...
    try {
      ...
      dispatch({ type: 'SET_ACTIVE_REPORT_JOB', payload: job });
      dispatch({ type: 'SET_LOADING', payload: false });
      addLog(data.deduplicated ? 'Reopened existing report job' : 'Report job queued');
  ...
  }, [state.rows, state.reportSettings, state.dataQuality.summary, state.activeReportJob, addLog, validateRows, monitorReportJob]);
  ...
  const evaluateDetectionRule = useCallback(async (rule) => {
    const res = await fetch(`${API_BASE}/rules/evaluate`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ rows: state.rows, rule }),
    });
    if (!res.ok) {
      const detail = await res.json().catch(() => ({}));
  ...
  const previewDocx = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE}/preview-docx`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(body),
        signal: controller.signal,
      ...
      dispatch({ type: 'SET_PREVIEW_SIGNATURE', payload: JSON.stringify(body) });
  ...
  const closePreview = useCallback(() => {
    ...
    dispatch({ type: 'SET_PREVIEW_BLOB', payload: null });
  }, []);
