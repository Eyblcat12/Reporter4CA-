...
const FIELD_OPTIONS = [
  ...
  ['software', 'Phần mềm'],
  ['process', 'Tiến trình'],
  ['installed_software', 'Phần mềm đã cài'],
  ['hostname', 'Hostname'],
  ['ip', 'IP'],
  ['os', 'Hệ điều hành'],
];
...
export default function RuleManager() {
  ...
  const exportRules = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = 'reporter-pro-rules.json';
      anchor.click();
  ...
  const importRules = async (event) => {
    ...
    try {
      ...
      setTeamMessage(
        `Đã import ${result.imported.length}; bỏ qua ${result.skipped.length}; lỗi ${result.errors.length}.`,
      );
      await fetchDetectionRuleConflicts();
    } catch (err) {
      setError(err.message);
    ...
    <div className="rule-manager">
      <div className="rule-manager__header">
        ...
        <div className="rule-manager__toolbar">
          <button type="button" onClick={exportRules} disabled={busy}>
      ...
      {showBuilder && (
        <section className="rule-builder" aria-label="Tạo rule phát hiện">
          <div className="rule-builder__title">
            <div>
              <strong>{draft.id ? 'Chỉnh sửa rule' : 'Rule mới'}</strong>
              <small>Chạy thử trên dữ liệu hiện tại trước khi lưu và áp dụng.</small>
            </div>
            <button type="button" aria-label="Đóng" onClick={() => setShowBuilder(false)}>
              <X size={16} />
            </button>
          </div>
          <div className="rule-builder__grid">
            <label>
              ...
              <input
                ...
                placeholder="Ví dụ: Phát hiện công cụ proxy nội bộ"
              />
            </label>
            <label>
              Phân loại
          ...
          <fieldset className="rule-builder__fields">
            <legend>Tìm trong trường</legend>
            {FIELD_OPTIONS.map(([value, label]) => (
              <label key={value}>
                <input
                  type="checkbox"
                  checked={draft.conditions.fields.includes(value)}
                  onChange={() => setConditionField(value)}
                />
                {label}
              </label>
            ))}
          ...
          <div className="rule-builder__grid rule-builder__grid--two">
            <label>
              ...
              <textarea
                ...
                placeholder="proxifier, tor browser, vpn client"
              />
              <small>Phân tách bằng dấu phẩy hoặc xuống dòng; chỉ cần khớp một từ.</small>
            </label>
            <label>
              Loại trừ khi có
              <textarea
                ...
                placeholder="được phê duyệt, hợp lệ, authorized"
              />
              <small>Giúp tránh cảnh báo nhầm từ nội dung phủ định hoặc đã xác minh.</small>
            </label>
          </div>
          <label>
            Hướng xử lý
            <input
              ...
              placeholder="Xác minh mục đích và phê duyệt sử dụng"
            />
          </label>
          ...
          {testResult && (
            <div className="rule-test">
              <div className="rule-test__summary">
                <span>
                  <Sparkles size={13} />
                  <b>{testResult.matchedRows}</b> khớp
                </span>
                <span>
                  <CheckCircle2 size={13} />
                  <b>{testResult.changedRows}</b> đổi kết luận
                </span>
                <span>
                  <b>{testResult.impact?.servers || 0}</b> server ·{' '}
                  <b>{testResult.impact?.clients || 0}</b> client
                </span>
              </div>
              {(testResult.conflicts || []).length > 0 && (
                <p className="rule-test__warning">
                  <AlertTriangle size={13} /> Rule nháp chồng lấn với {testResult.conflicts.length}{' '}
                  rule hiện có.
                </p>
              )}
              {testResult.matches.length ? (
                <ul>
                  {testResult.matches.slice(0, 12).map((match) => (
                    <li key={`${match.row}-${match.hostname}`}>
                      <b>
                        Dòng {match.row + 1} · {match.hostname || 'Chưa có hostname'}
                      </b>
                      <span>
                        {match.assessmentBefore} → {match.assessmentAfter}
                      </span>
                      <small>
                        {match.evidence.map((item) => `${item.field}: ${item.matched}`).join('; ')}
                      </small>
                    </li>
                  ))}
                </ul>
          ...
          {draft.id && versions.length > 0 && (
            <div className="rule-versions">
              <strong>
                  ...
                  <span key={item.id}>
                    ...
                    {index > 0 && (
                      <button
                        type="button"
                        disabled={busy}
                        onClick={() => rollback(item.versionNumber)}
                      >
                        Khôi phục
                      </button>
                    )}
          ...
          <div className="rule-builder__actions">
            <button type="button" onClick={runTest} disabled={busy || !rows?.length}>
              <FlaskConical size={14} /> Thử trên {rows?.length || 0} dòng
            </button>
          ...
          return (
            <div className="rule-manager__item" key={rule.id}>
              ...
              <span className="rule-manager__content">
                ...
                <small>
                  {rule.source === 'custom' ? 'Tùy chỉnh' : 'Mặc định'} · {rule.severity} ·{' '}
                  {rule.classification}
                </small>
