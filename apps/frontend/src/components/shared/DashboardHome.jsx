...
function buildSummary(history, days, locale) {
  ...
  const completed = current.filter(
    (item) => getField(item, 'status', 'status', 'success') === 'success',
  );
  const failed = current.filter(
    (item) => getField(item, 'status', 'status', 'success') === 'failed',
  ).length;
  const cancelled = current.filter(
    (item) => getField(item, 'status', 'status', 'success') === 'cancelled',
  ).length;
  const assets = completed.reduce((sum, item) => sum + reportRows(item), 0);
...
export default function DashboardHome({ onOpenImport }) {
  ...
  return (
    ...
    >
      ...
      {dashboardState.status === 'degraded' && (
        <section className="dashboard-home__notice" role="status" aria-live="polite">
          <span className="dashboard-home__notice-icon">
            <WifiOff size={18} />
          </span>
          <div>
            <strong>{t('dashboard.degradedTitle')}</strong>
            <span>{t('dashboard.degradedBody')}</span>
          </div>
          <button
      ...
      <section
        className={`dashboard-home__kpis ${isInitialLoading ? 'dashboard-home__kpis--loading' : ''}`}
        ...
        <section className="dashboard-home__activity" aria-labelledby="dashboard-activity-title">
          ...
          <div className="dashboard-home__chart-detail" aria-label={t('dashboard.metrics')}>
            <span className="dashboard-home__legend-item dashboard-home__legend-item--success">
              <i />
              {t('dashboard.successful')}: {numberFormatter.format(summary.reports || 0)}
            </span>
            <span className="dashboard-home__legend-item dashboard-home__legend-item--failed">
              <i />
              {t('dashboard.failed')}: {numberFormatter.format(summary.failed || 0)}
            </span>
            <span className="dashboard-home__legend-item dashboard-home__legend-item--cancelled">
              <i />
              {t('dashboard.cancelled')}: {numberFormatter.format(summary.cancelled || 0)}
            </span>
            <span className="dashboard-home__latest-period">
      ...
      <footer className={`dashboard-home__status dashboard-home__status--${dashboardState.status}`} aria-live="polite">
        ...
        {dashboardState.status === 'loading' || dashboardState.status === 'refreshing'
          ? t('dashboard.loading')
          : t('dashboard.ready')}
        {canFormatUpdatedAt && dashboardState.status !== 'degraded' && (
          <time dateTime={updatedAt.toISOString()}>
            {t('dashboard.lastUpdated')} {syncFormatter.format(updatedAt)}
          </time>
        )}
      </footer>
    </motion.div>
