...
describe('DashboardHome', () => {
  ...
  it('explains degraded data and retries both dashboard endpoints', async () => {
    const user = userEvent.setup();
    reporter.fetchReportHistory.mockResolvedValue({ ok: true, data: { reports: [] } });
    reporter.fetchDashboardSummary.mockResolvedValue({ ok: false, status: 503 });
    render(<DashboardHome />);

    expect(await screen.findByText('Latest dashboard data is unavailable')).toBeInTheDocument();
    reporter.fetchReportHistory.mockClear();
    reporter.fetchDashboardSummary.mockClear();
